...
import numpy as np
import sys
filepath = 'data/synth_mult/mult2'
lens = [10,30,100,400]
if len(sys.argv)>1:
    lens = [int(sys.argv[1])]
ends = ['.train','.test']
...
for l in lens:
    for end in ends:
        ...
        for count in range(1000):
            time_scales = []
            for i in range (int(ev_types)):
                time_scales.append(4**i)
            countall += 1
            id=str(countall).zfill(5)
            events = []
            t = 0
            for ev in range(int(ev_types)):
                ts = time_scales[ev]
            ...
            c = 0
            while c<=l+1:
                ev = random.randint(1,1)
                events.append([ev+ev_types,t])
                t += events[l-1][1]/l*ev_types
                c+=1
            events.sort(key=lambda x: x[1])
            events = events[:l+1]
            time1 = [str(0.0)]
