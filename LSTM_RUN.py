...
ops = {
            ...
            'encoder': 'LSTM',
            'dataset': 'data/synth_accum/accum_nonunif0.25',
            'overwrite': False,
...
if ops['max_length'] == "ALL" or ops['task'] == 'CLASS':
    ops['max_length'] = DH.longest_seq([train_set,valid_set,test_set])
if ops['embedding']:
...
with tf.device(ops['device']):
    ...
    while epoch < ops['epochs']:
        ...
        if ops['write_history'] and epoch==ops['epochs']:
            DH.write_history(accuracy_entry, 'records/accLSTMaccum0.25_nonunif.txt', epoch, ops['overwrite'])
